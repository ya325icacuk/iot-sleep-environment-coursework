...
# ============================================================
    ...
    with st.container(border=True):
        ...
        for col_name, label, unit, less_word, more_word in range_vars:
            ...
            label_display = f"{label} ⭐" if is_strongest else label
            rows_data.append({
                "Condition": label_display,
                "Good Nights Avg": f"{g_mean:.1f}{unit}",
                "Poor Nights Avg": f"{p_mean:.1f}{unit}",
        ...
        table_html = """<table style="width: 100%; border-collapse: collapse; font-size: 1.5rem;">
            <thead>
                <tr style="border-bottom: 2px solid rgba(148, 163, 184, 0.2);">
                    <th style="text-align: left; padding: 1rem 1.3rem; color: #64748B; font-weight: 600;">Condition</th>
                    ...
                    <th style="text-align: left; padding: 1rem 1.3rem; color: #64748B; font-weight: 600;">Difference</th>
                </tr>
            </thead>
            <tbody>"""
        for row in rows_data:
            table_html += f"""
                <tr style="border-bottom: 1px solid rgba(148, 163, 184, 0.1);">
                    <td style="padding: 1rem 1.3rem; color: #475569; font-weight: 500;">{row["Condition"]}</td>
                    ...
                    <td style="padding: 1rem 1.3rem; color: #475569;">{row["Difference"]}</td>
                </tr>"""
    ...
    with st.container(border=True):
        ...
        st.markdown(f"""<div style="background: rgba(184, 154, 222, 0.08); border-radius: 12px; padding: 1.75rem 2rem; margin-bottom: 1rem;">
